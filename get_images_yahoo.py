...
def fetch_and_save_img(word):
    data_dir = 'image/'
    data_plant_dir = '{}'.format(word)

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)

    if not os.path.exists(data_dir + data_plant_dir):
        os.makedirs(data_dir + data_plant_dir)
    ...
    for i, img_url in enumerate(img_url_list(word)):
        ...
        if not ext:
            continue
        result_file = os.path.join(data_dir + data_plant_dir, str(i) + ext)
...
if __name__ == '__main__':
    word = sys.argv[1]
    fetch_and_save_img("{}".format(word))
